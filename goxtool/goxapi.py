...
class BaseClient(BaseObject):
    ...
    def _try_send_raw(self, raw_data):
        ...
        if self.connected:
            try:
                #self.debug("Sending raw data: %s" % raw_data)
...
#already subscribed to on startup
...
#CHANGED MESSAGE FORMAT        # self.debug(" tick:  bid:", int2str(bid, self.currency),
...
#CHANGED MESSAGE FORMAT       # self.debug(
...
class OrderBook(BaseObject):
    ...
    def slot_user_order(self, dummy_sender, data):
        ...
        self.signal_changed(self, None)
        self.ownorders_changed(self, None)
